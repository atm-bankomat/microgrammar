...
import { MatchingMachine, Microgrammark } from "../src/Microgrammar";
...
describe("Microgrammar", () => {
    ...
    it("allows valid call to function", () => {
        const content = "foo ";
        const validMg = Microgrammark.fromDefinitions({
            content: makeMg(),
        });
    ...
    it("stop after match with arrow function", () => {
        ...
        const mg = Microgrammark.fromDefinitions<Named>({
            name: /[A-Z][a-z]+/,
        });
    ...
    it("flatten definitions into parent", () => {
        ...
        const mg = Microgrammark.fromDefinitions<CatsDogsAndPigs>({
            dogs: names,
            _separator: "****",
            cats: names,
            _separator2: "****",
...
class StringGrammar {
    ...
    public static readonly stringGrammar: Microgrammark<any> = Microgrammark.fromDefinitions<any>({
        foo: ctx => {
