...
describe("MicrogrammarFromString", () => {
    ...
    it("literal", () => {
        const content = "foo ";
        const mg = Microgrammart.fromString("foo");
    ...
    it("XML element", () => {
        const content = "<foo>";
        const mg = Microgrammart.fromString("<${name}>", {
            name: /[a-zA-Z0-9]+/,
        });
        const result = mg.findMatches(content);
        // console.log("Result is " + JSON.stringify(result));
    ...
    it("2 XML elements via nested microgrammar", () => {
        ...
        const element = Microgrammart.fromString("<${namex}>", {
            namex: /[a-zA-Z0-9]+/,
        });
        const mg = Microgrammart.fromString("${first}${second}", {
            first: element,
            second: element,
        });
        const result = mg.findMatches(content);
    ...
    it("2 elements: whitespace insensitive", () => {
        ...
        const mg = Microgrammart.fromString<{ namex: string }>("<${namex}> notxml", {
            namex: /[a-zA-Z0-9]+/,
        });
        const result = mg.findMatches(content);
        assert(result.length === 1);
    ...
    it("2 elements: whitespace sensitive: match with return", () => {
        const content = "<first>\n\tnotxml";
        const mg = Microgrammart.fromString<{ namex: string[] }>("<${namex}>\n\tnotxml", {
            namex: /[a-zA-Z0-9]+/,
        });
        const result = mg.findMatches(content);
        assert(result.length === 1);
    ...
    it("stop after match", () => {
        ...
        const mg = Microgrammart.fromString<Named>("${name}", {
            name: /[A-Z][a-z]+/,
        });
    ...
    function dependencyGrammar() {
        return Microgrammart.fromString<VersionedArtifact>(
            `<dependency>
    ...
    function namesGrammar() {
        ...
        return Microgrammart.fromString("${dogs} **** ${cats}", {
            dogs: names,
            cats: names,
        });
    }
    ...
    it("handles escaped literal $", () => {
        ...
        const mg = Microgrammart.fromString("The \$${currency} has risen lately", {
            currency: /[A-Z]{3}/,
        });
        const result = mg.findMatches(content);
        assert(result.length === 1);
    ...
    it("uses dictionary in long sentence", () => {
        ...
        const mg = Microgrammart.fromString<any>("The recent movement in the ${currency} has gained you $${amount}", dictionary);
