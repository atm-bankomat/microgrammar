 ...
 * afterwards.
 */
export class Microgrammart<T> implements Term {
    ...
    public static fromDefinitions<T>(definitions: {}): Microgrammart<T> {
        return new Microgrammart<T>(Concat.of(definitions));
    }

    public static fromString<T>(spec: string,
                                components: object = {},
                                options: FromStringOptions = {}): Microgrammart<T> {
        return new Microgrammart<T>(
...
export abstract class MatchingMachine {
    ...
    public consume(input: string | InputStream, parseContext = {}, l?: Listeners): void {
        const omg = this.observer ? Microgrammart.fromDefinitions(this.observer) : undefined;
